...
export function StatusBadge({ label, tone = "neutral" }: { label: string; tone?: Tone }) {
  return (
    <span className={clsx("inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-xs font-medium", toneClasses[tone])}>
      {label}
    </span>
  );
}
...
export function statusToTone(status: string): Tone {
  const map: Record<string, Tone> = {
    completed: "success", active: "info", needs_response: "warning", waiting: "neutral",
    overdue: "danger", due_today: "warning", due_this_week: "info", escalated: "danger",
    confirmed: "success", dismissed: "neutral", pending_review: "warning", pending: "warning",
    needs_context: "info", open: "info", strong: "success", steady: "info", at_risk: "danger",
  };
      ...
      {open && (
        <div className="absolute z-30 mt-2 w-72 rounded-lg border border-[var(--color-line)] bg-white p-3 shadow-lg text-left">
          ...
          <p className="mt-1 text-xs text-[var(--color-ink-700)] leading-relaxed">
            {reasoning ?? "SpikeOS analyzed communication patterns and identified this as a potential coaching opportunity."}
          </p>

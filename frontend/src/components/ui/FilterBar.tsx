      ...
      {tabs.map((t) => (
        <button
          ...
          className={`relative px-3 py-2 text-sm font-medium transition-colors ${
            active === t.key ? "text-[var(--color-blue-600)]" : "text-[var(--color-ink-500)] hover:text-[var(--color-ink-900)]"
          }`}
        >
          {t.label}
          {typeof t.count === "number" && (
            <span className="ml-1.5 rounded-full bg-[var(--color-surface)] px-1.5 py-0.5 text-[11px]">{t.count}</span>
          )}
          {active === t.key && <span className="absolute -bottom-px left-0 right-0 h-0.5 bg-[var(--color-blue-600)]" />}
        </button>
      ))}
    </div>
  );

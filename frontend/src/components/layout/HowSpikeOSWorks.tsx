...
function Node({ title, subtitle, accent }: { title: string; subtitle?: string; accent?: boolean }) {
  return (
    <div className={`rounded-lg border px-4 py-2.5 text-center text-xs font-medium ${accent ? "border-[var(--color-blue-500)] bg-[var(--color-blue-50)] text-[var(--color-blue-600)]" : "border-[var(--color-line)] bg-white text-[var(--color-ink-900)]"}`}>
      {title}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <ArrowDown size={16} className="text-[var(--color-ink-400)]" />
        <Node title="SpikeOS Intelligence (FastAPI)" accent />
        ...
        <div className="flex items-center gap-2 text-[var(--color-ink-400)]"><ArrowRight size={14} /></div>
      ...
      <p className="rounded-md bg-[var(--color-surface)] p-3 text-xs text-[var(--color-ink-500)]">
        This is a demo visualization of the intended production architecture — the FastAPI
        backend in this repository is real, but Microsoft Graph and Power BI are not yet

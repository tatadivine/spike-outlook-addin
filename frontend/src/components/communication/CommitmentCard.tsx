...
const statusLabel: Record<Commitment["status"], string> = {
  active: "Active", due_today: "Due today", due_this_week: "Due this week", overdue: "Overdue", completed: "Completed",
};
  ...
  return (
    <Card>
      ...
      <div className="mt-3 flex items-center justify-between text-xs text-[var(--color-ink-500)]">
        <span>Due {new Date(commitment.due_date).toLocaleDateString()}</span>
        {commitment.days_overdue > 0 && <span className="text-[var(--color-red-600)]">{commitment.days_overdue}d overdue</span>}
      </div>
      <p className="mt-2 text-xs text-[var(--color-ink-700)]">{commitment.next_action}</p>
    </Card>
  );
}

...
export function ResponseTrendCard({
  data, goal = 90, employeeLabel = "Employee", teamLabel = "Team Average", weeksAboveGoal,
}: { data: TrendWeek[]; goal?: number; employeeLabel?: string; teamLabel?: string; weeksAboveGoal?: string }) {
    ...
    <Card>
      <div className="mb-3 flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-3 pt-0.5 text-[11px] font-medium text-[var(--color-ink-500)]">
          <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[var(--color-blue-600)]" /> {employeeLabel}</span>
          <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-[var(--color-teal-600)]" /> {teamLabel}</span>
          <span className="flex items-center gap-1.5 text-[var(--color-ink-400)]"><span className="h-0 w-3 border-t-2 border-dashed border-[var(--color-ink-400)]" /> Goal ({goal}%)</span>
        </div>
      </div>
        ...
        <div className="h-[220px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 8, right: 12, left: -18, bottom: 0 }}>
              ...
              <YAxis domain={[60, 100]} tickFormatter={(v) => `${v}%`} tick={{ fontSize: 11, fill: "#8892a0" }} axisLine={false} tickLine={false} width={38} />
              <ReferenceLine y={goal} stroke="#8892a0" strokeDasharray="4 4" />
              <Tooltip formatter={(v) => `${v}%`} contentStyle={{ borderRadius: 8, border: "1px solid #e3e7ee", fontSize: 12 }} />

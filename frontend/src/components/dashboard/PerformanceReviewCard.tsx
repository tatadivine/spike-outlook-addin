...
export function PerformanceReviewCard({
  overallLabel, deltaPoints, filledDots, totalDots = 7, strengths, coaching,
}: { overallLabel: string; deltaPoints: number; filledDots: number; totalDots?: number; strengths: ReviewPoint[]; coaching: ReviewPoint[] }) {
  return (
    <Card>
        ...
        <div className="text-right">
          ...
          <div className="mt-1.5 flex justify-end gap-1">
            {Array.from({ length: totalDots }).map((_, i) => (
              <span key={i} className={clsx("h-2.5 w-2.5 rounded-full", i < filledDots ? "bg-[var(--color-green-600)]" : "bg-[var(--color-line)]")} />
            ))}
          </div>
        </div>
      </div>

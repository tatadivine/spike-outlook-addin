...
export function MetricStatCard({
  icon: Icon, tone, label, value, suffix, trend, trendLabel, progressPct, source,
}: {
  icon: LucideIcon; tone: Tone; label: string; value: string | number; suffix?: string;
  trend: "up" | "down" | "flat"; trendLabel: string; progressPct: number; source?: string;
}) {
  ...
  return (
    <Card className="relative">
      <div className="flex items-start justify-between gap-2">
        ...
        {source && (
          <button aria-label={`About ${label}`} onClick={() => setOpen((v) => !v)} className="mt-0.5 text-[var(--color-ink-400)] hover:text-[var(--color-blue-600)]">
            <Info size={13} />
          </button>
        )}
      </div>
      ...
      <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-[var(--color-line)]">
        <div className={clsx("h-full rounded-full", TONE_BG[tone])} style={{ width: `${Math.max(4, Math.min(100, progressPct))}%` }} />
      </div>

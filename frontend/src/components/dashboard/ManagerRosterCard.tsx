...
export function ManagerRosterCard({
  employees, subtitle, onSelect, emptyLabel,
}: { employees: RosterEmployee[]; subtitle: string; onSelect: (employeeId: string) => void; emptyLabel?: string }) {
  return (
    <Card>
      ...
      {employees.length === 0 ? (
        ...
        <table className="w-full text-left text-[13px]">
          ...
          <tbody>
            {employees.map((e) => {
              const trend = e.response_score >= 85 ? "up" : e.response_score < 70 ? "down" : "flat";
              return (
                <tr key={e.id} onClick={() => onSelect(e.id)} className="cursor-pointer border-t border-[var(--color-line)] hover:bg-[var(--color-surface)]">
                  <td className="py-3">
                  ...
                  <td className="py-3 font-semibold" style={{ color: e.response_score >= 85 ? "var(--color-green-600)" : e.response_score >= 75 ? "var(--color-amber-600)" : "var(--color-red-600)" }}>
                    {e.response_score}
                  </td>
                  <td className="py-3" style={{ color: e.overdue_follow_ups > 8 ? "var(--color-red-600)" : "var(--color-ink-900)" }}>
                    {e.overdue_follow_ups}
                  </td>
                  <td className="py-3">

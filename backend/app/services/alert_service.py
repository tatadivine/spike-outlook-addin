...
def list_for_identity(identity: Identity, owner_id: str | None = None) -> list[dict]:
    ...
    if not target_owner:
        return gen.alerts if identity.role == "administrator" else []
    if not permissions_service.can_view_employee(identity.role, identity.employee_id, target_owner):
        raise forbidden("You are not authorized to view this employee's alerts.")
    return gen.alerts_for_owner(target_owner)
